DROP FUNCTION IF EXISTS resolve_camera_locations();

CREATE OR REPLACE FUNCTION resolve_camera_locations()
    RETURNS location_resolution_run
    LANGUAGE plpgsql
AS
$$
DECLARE
    v_run                     location_resolution_run%ROWTYPE;
...
BEGIN
    -- Step 1. create new location_resolution_run row to track
    INSERT INTO location_resolution_run (run_start,
                                         run_result)
    VALUES (now(),
            'PENDING')
    RETURNING *
    ...
    -- Step 3. Resolve candidate locations
    -- Join suburb first, then the street within that suburb.
    ...
    -- Step 3.1 Persist only successful matches as relationship rows.
    INSERT INTO resolved_camera_location (location_id,
                                          resolution_run_id,
                                          street_by_suburb_id)
    SELECT match.location_id,
           v_run.resolution_run_id,
           match.streets_by_suburb_id
    FROM tmp_camera_resolution_matches match;

    GET DIAGNOSTICS v_matched_count = ROW_COUNT;

    -- Step 3.2 Any candidate not present in the matched set is treated as unresolved for this run.
    ...
    -- Mark the run as successful even if some rows were unresolved.
    ...
    WHERE resolution_run_id = v_run.resolution_run_id
    RETURNING *
        INTO v_run;

    RAISE NOTICE 'resolve_camera_locations updated % rows in run %',
        v_matched_count,
        v_run.resolution_run_id;
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        IF v_run.resolution_run_id IS NOT NULL THEN
            UPDATE location_resolution_run
            SET run_end = NOW(),
                run_result = 'FAIL',
                meta = jsonb_build_object(
...
-- BEGIN;
-- SELECT * FROM resolve_camera_locations();
-- ROLLBACK;